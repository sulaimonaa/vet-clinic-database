...
SELECT v.name, s.specialization
FROM vets v
...
SELECT v.animal, vet.name AS vet_name, v.visit_date
FROM visits v
JOIN vets vet ON v.vet_id = vet.id
ORDER BY v.visit_date DESC
LIMIT 1;
...
WHERE s.specialization IS NULL OR s.specialization NOT LIKE CONCAT('%', a.species_id, '%');
